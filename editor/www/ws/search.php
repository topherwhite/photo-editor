...
header('Cache-Control: no-cache, must-revalidate'); header('Expires: Fri, 1 Jan 2010 00:00:00 GMT');
...
//	record_to_log("search",$ws,$con);
	...
	{	if ($input_vars["bucket_id"] == "nobucket")
		...
		{	$qu_from = "editor.bucket_image JOIN (editor.image) ON (editor.bucket_image.image_id=editor.image.image_id)";
			$rtrn_fields['bucket_image'] = array("bucket_id","position_x","position_y","position_z"/*,"position_wd","position_ht"*/,"rank");
			$order_by = "rank ASC";
		}
	}
	else { $qu_from = "editor.image"; }

	$rtrn_fields['image'] = array("origin_id","width","height","rating","rotation");
	...
	$qu[0] .= " ORDER BY {$order_by} LIMIT {$var['ws']['return_limit']}";
