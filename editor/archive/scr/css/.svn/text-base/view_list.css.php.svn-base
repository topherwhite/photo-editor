...
for ($i = 0; $i < ($img_disp['tble']['per_row']*$img_disp['tble']['num_rows']); $i++)
{
	...
	$info['cell']['x'] = 0;//($n*($img_disp['size']['thmb']+(2*$img_disp['tble']['pad'])+5));
	$info['cell']['y'] = ($i*($img_disp['size']['thmb']+(2*$img_disp['tble']['pad'])+5));
	...
	echo ""
		."\ndiv.popup_view_{$i}, div.popup_meta_{$i} {"
					...
					." top:".($info['cell']['y']+($img_disp['size']['thmb']+(2*$img_disp['tble']['pad'])))."px;"
					." }"
		...
		."\ndiv.meta_cell_{$i} {"
					." background-color:transparent; position:absolute; z-index:0; border:solid 1px #ffffff; overflow:visible;"
					." width:940px;padding:none;"
					." height:".($img_disp['size']['thmb']+(2*$img_disp['tble']['pad']))."px;"
					." left:".($info['cell']['x']+5+$img_disp['size']['thmb']+(2*$img_disp['tble']['pad']))."px;"
					." top:".($info['cell']['y'])."px;"
					." }"
					
		."\ntextarea.text_cptn {"
					." position:absolute; width:50%; left:2px; top:2px; padding:10px; border:none;"
					." height:".($img_disp['size']['thmb']+(2*$img_disp['tble']['pad'])-26)."px;"
					." }"

		."\ndiv.popup_meta_show_{$i} {"
					." position:absolute; background-color:#cccccc; height:auto; width:auto; color:black; font-size:9px;"
	 				." border:solid 2px #f78f1e; visibility:visible; z-index:10; padding:20px 20px 20px 20px;"
					." width:".(300)."px;"
					." min-height:".($img_disp['size']['thmb']+(2*$img_disp['tble']['pad'])+20)."px;"
					." left:".(500)."px;"
		...
		."\ndiv.popup_notes_{$i} {"
					...
					." top:".($info['cell']['y']+($img_disp['size']['thmb']+(2*$img_disp['tble']['pad'])))."px;"
					." }"
		."\ndiv.popup_notes_{$i} textarea {"
					." position:absolute; width:".(500-20)."px; height:200px; left:10px; top:10px; padding:20px; border:none;"
					."}"
