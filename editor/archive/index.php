...
require_once("/liz/www/archive/scr/inc/popup.inc.php");
...
echo "\n<link href=\"scr/css/view_{$stylesheet}.css?key=".hash('md5',serialize($img_disp))."\" rel=\"stylesheet\" type=\"text/css\" />";
...
echo 	"<body onLoad=\""
		...
		."<input type=\"button\" class=\"apply\" value=\"upload new image\" onClick=\"".popup_bttn(1,"upload")."\" />"
		
		.popup_open(1,"upload",10)
		...
		."<input type=\"button\" class=\"apply\" value=\"xml (read)\" style=\"float:right;\" onClick=\"window.open('/scr/php/meta_feed2?fltr={$_GET['fltr']}')\" />"
...
echo 	"<option value=\"xxxxxxxxxxxx\"{$alb_sel_trash}>- trash bin -</option>"
		...
		."<br /><br /><label for=\"view_slct\">filter: </label>"
			...
			."<option value=\"img*status\"{$fltr['slct']['img*status']}>status</option>"
...
echo 	"<br /><br /><label for=\"batch_rating_slct\">apply rating to selected: </label>"
		...
		."<br /><br /><label for=\"batch_status_slct\">apply workflow status to selected: </label>"
		."<select id=\"batch_status_slct\" name=\"batch_status_slct\""
		." onChange=\"document.getElementById('batch_img_status').value"
				."=document.getElementById('batch_status_slct').options"
					."[document.getElementById('batch_status_slct').selectedIndex].value\">"
			."<option value=\"0\">-</option><option value=\"1\">completed</option><option value=\"2\">exported</option>"
			."</select>"
		."<input type=\"hidden\" id=\"batch_img_status\" value=\"0\" />"
		."<input type=\"button\" class=\"apply\" value=\"->\""
			." onClick=\"gather_checked('batch_apply','{$usr}','img_status')\" />"
...
//	$info['cell']['y'] = ($r*($img_disp['size']['thmb']+(2*$img_disp['tble']['pad'])+5));
	...
	$info['cell']['status'] = array(0=>"",1=>"",2=>"");
	$info['cell']['status'][$img['status']] = " selected=\"selected\"";	
	
	if (!empty($is_in_alb[$img['time']])) { $is_in_alb['str'] = "visible"; } else { $is_in_alb['str'] = "hidden"; }
	if ($img['status'] != 0) { $status_advanced['str'] = "visible"; } else { $status_advanced['str'] = "hidden"; }
...
echo	""
		...
		.popup_open($slct_id,"meta",($view_per_page-$slct_id))	
			...
			."<br /><br />"
			."<input class=\"apply\" type=\"button\" value=\"edit metadata on separate screen...\" onClick=\"location='/metadata?wh={$img['time']}';\" />"
			."<br /><br />"
...
{			
echo "<br /><br />"
		."<select class=\"status\" name=\"status_{$slct_id}\" id=\"status_{$slct_id}\" onChange=\"req_status_update({$slct_id})\">"
			."<option value=\"0\"{$info['cell']['status'][0]}>-</option>"
			."<option value=\"1\"{$info['cell']['status'][1]}>completed</option>"
			."<option value=\"2\"{$info['cell']['status'][2]}>exported</option>"
		."</select>"
		."<label for=\"status_{$slct_id}\" style=\"float:right;\">workflow status: </label>"		
		;	
}			
...
echo 	popup_close()
		...
		."<div class=\"status_advanced\" id=\"status_advanced_{$slct_id}\" style=\"visibility:{$status_advanced['str']};\"></div>"		
	...
	//		."<a class=\"download\" target=\"download\" href=\"img/dynamic/down.{$img['creator']}.{$img['time']}.jpg\">"
			...
			." onClick=\"".popup_bttn($slct_id,"view")."\""
	...
	//		."</a>"
		...
		//		." onClick=\"location='/metadata?wh={$img['time']}';\""
				." onClick=\"".popup_bttn($slct_id,"meta")."\""
		...
		."<input type=\"hidden\" id=\"dflt_status_{$slct_id}\" value=\"{$img['status']}\" />"
...
if ($stylesheet == "list")
{
	...
	echo "\n<div class=\"meta_cell_{$slct_id}\" id=\"meta_{$slct_id}\">"
		...
		."<br /><br />"
		."<input class=\"apply\" style=\"float:right;\" type=\"button\" value=\"view original meta\" onClick=\"".popup_bttn($slct_id,"meta_show")."\" />"
		."<br /><br />"
		."<input class=\"apply\" style=\"float:right;\" type=\"button\" value=\"write/edit notes\" onClick=\"".popup_bttn($slct_id,"notes")."\" />"
		."<br /><br />"
		."<select class=\"status\" name=\"status_{$slct_id}\" id=\"status_{$slct_id}\" onChange=\"req_status_update({$slct_id})\">"
			."<option value=\"0\"{$info['cell']['status'][0]}>-</option>"
			."<option value=\"1\"{$info['cell']['status'][1]}>completed</option>"
